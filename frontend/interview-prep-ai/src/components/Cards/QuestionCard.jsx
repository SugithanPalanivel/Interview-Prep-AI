...
const QuestionCard = ({
  ...
  useEffect(()=>{
    if(isExpanded){
      const contentHeght=contentRef.current.scrollHeight;
      setHeight(contentHeght + 10)
    }
    else{
      setHeight(0);
    }
  ...
  <div className='bg-white rounded-lg mb-4 overflow-hidden py-4 px-5 shadow-xl shadow-gray-100/70 border border-gray-100/60 group'>
      ...
      <div className='flex items-center justify-end ml-4 relative'>
        ...
        <button 
         ...
         >
          <LuChevronDown
             size={20}
             className={`transform trabsition-transform duration-300 ${
              isExpanded ? "rotate-100" :""
             }`} />
    ...
    >
      ...
      >
        ...
        <AIResponsePreview content={typeof answer === "string" ? answer : JSON.stringify(answer)} />
